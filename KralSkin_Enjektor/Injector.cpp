	...
	if (::K32EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded)) {
		for (auto i{ 0u }; i < (cbNeeded / sizeof(HMODULE)); ++i) {
			...
			if (::K32GetModuleBaseNameW(hProcess, hMods[i], szModName, sizeof(szModName) / sizeof(TCHAR))) {
				if (::wcscmp(szModName, L"KralSkin.dll") == 0) {
...
bool WINAPI Injector::inject(const std::uint32_t pid) noexcept
{
	...
	const auto dll_path{ std::wstring(current_dir) + L"\\KralSkin.dll" };

	if (auto f{ std::ifstream(dll_path) }; !f.is_open()) {
		MessageBox(nullptr, L"KralSkin.dll dosyası bulunamadı.\nHileyi yeniden yüklemeyi deneyin.", L"KralSkin", MB_ICONERROR | MB_OK);
		::CloseHandle(handle);
		return false;
	}
...
void Injector::run() noexcept
{
	...
	while (true) {
		...
		KralSkinEnjektor::gameState = (league_processes.size() > 0) ? true : false;
		KralSkinEnjektor::clientState = (league_client_processes.size() > 0) ? true : false;

		for (auto& pid : league_processes) {
			if (!Injector::isInjected(pid)) {
				KralSkinEnjektor::cheatState = false;
				if (KralSkinEnjektor::btnState) {
					std::this_thread::sleep_for(1.5s);
					if (Injector::inject(pid))
						KralSkinEnjektor::cheatState = true;
					else
						KralSkinEnjektor::cheatState = false;
				}
